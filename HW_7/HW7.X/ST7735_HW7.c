...
void LCD_drawString(unsigned short x, unsigned short y, char* message, unsigned short colorOn, unsigned short colorOff){
    int i = 0;
    while (message[i]){                                         //checking for /0 null char
        LCD_drawChar(x+(5*i), y, message[i], colorOn, colorOff);  //shift col by 5i to draw the character
        i++;                                                    //move to next character in message array
    }
}

void LCD_drawChar(unsigned short x, unsigned short y, char character, unsigned short colorOn, unsigned short colorOff){
    int col;
    int row;
    char pixels;
    
    for (col=0; col<5; col++){                          //for each column run through this loop
        pixels = ASCII[character-0x20][col];           //shift char because 0th row of ASCII array is at hex20
        for(row=0; row<8; row++){                       //go through each row for each column
            //if(x<col<128 & y<row<160){                  //check for allowed pixel dimensions 128x160
                if((pixels>>row) & 1){                    //checking if pixel is on ??not sure about the and 1
                    LCD_drawPixel(x+col, y+row, colorOn);   //draw the color for pixel on
                } else{                                 //otherwise draw the off color
                    LCD_drawPixel(x+col, y+row, colorOff);
                }
            //}
        }
    }
...
void LCD_drawProgressBar(unsigned short x, unsigned short y, unsigned short height, unsigned short len1, unsigned short c1, unsigned short c2){
    int col;
    int row;
    ...
    for(col=0; col<=100; col++){
        for(row=0; row<=height; row++){
            if(col>len1){
                LCD_drawPixel(x+col, y+row, c1);
            } else{
                LCD_drawPixel(x+col, y+row, c2);
            }
        }
    }
}
