#include "ST7735.h"      //to use LCD functions
...
int main(void) {
	...
	// remember the core timer runs at half the sysclk
        ...
        */
        ...
        for(j=0; j<101; j++){
            _CP0_SET_COUNT(0);
            sprintf(message, "Hello World! %d",j);
            LCD_drawString(1, 1, message, BLACK, WHITE);
            LCD_drawProgressBar(10, 25, 5, j, YELLOW, GREEN); 
            if(j>=100){
                j=0;
                LCD_drawString(1, 1, "Hello World! 0  ", BLACK, WHITE);
            }
